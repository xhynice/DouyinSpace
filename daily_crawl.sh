...
#
# 流程(每天 3 步):
#   1. 采集数据:每个用户最多 6 个视频 + 其全部评论/回复,只入 DB 不下载媒体
...
if python scripts/build_comment.py --sqlite --data-dir /data2/DouyinComment/data --cdn "https://huggingface.co/buckets/sunset139/douyin/resolve" >> "$LOG_FILE" 2>&1; then
    log "[5/5] 评论前端构建完成"
else
    log "[5/5] 评论前端构建失败"
fi
